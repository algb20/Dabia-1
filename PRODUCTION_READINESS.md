...
## ✅ Batch 18 — Production hardening (notifications, moderation, money, search, analytics)
- **Persistent notifications:** real `notifications` table generated by DB
  triggers on actual events — new follower, new message, dispute opened/resolved,
  order status change, and posts from accounts whose bell you enabled. Persisted
  read state, unread badge from `notifications_unread`, mark-all-read on open,
  and tappable notifications that deep-link (profile, chat, orders, wallet).
  Access is private via participant-checked RPCs.
- **Content moderation:** `content_reports` + report action on posts (reason
  sheet) → notifies admin. Admin panel gains a Reports section: remove content,
  mark actioned, or dismiss (`report_content`, `admin_list_reports`,
  `admin_resolve_report`, admin authorized by email).
- **Real money loop:** confirming receipt now **credits the seller's wallet**
  (SECURITY DEFINER trigger, guard-safe) with a ledger entry + notification.
  Sellers **request withdrawals** (`request_withdrawal` debits/holds balance);
  admin processes them, recording the **real Pi A2U transaction id**
  (`mark_withdrawal_paid`) or rejects+refunds. Verified end-to-end on the live DB.
  *(Fully-automated A2U payout still needs the app wallet secret + live testing;
  the admin-assisted flow is the safe production path today.)*
- **Unified global search:** the search overlay now returns **People +
  Communities + Products** together (plus the existing image & voice search).
- **Seller analytics:** real `products.view_count` incremented on each product
  open (`increment_product_view`); per-product **views** (and sold/rating) shown
  in My Products.
